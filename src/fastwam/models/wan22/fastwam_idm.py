...
class FastWAMIDM(FastWAMJoint):
    ...
    def infer_joint(
        ...
        #--------------------------------
        if os.environ.get("FASTWAM_ATTN_LAYOUT_DEBUG", "0") == "1":
            import json
            from pathlib import Path

            out_dir = Path(os.environ.get("FASTWAM_ATTN_DIR", "attention_outputs")) / "layout_debug"
            out_dir.mkdir(parents=True, exist_ok=True)

            meta = {
                "latents_video_shape": tuple(latents_video.shape),
                "latents_action_shape": tuple(latents_action.shape),
                "video_tokens_shape": tuple(video_pre_cond["tokens"].shape),
                "video_tokens_per_frame": int(video_pre_cond["meta"]["tokens_per_frame"]),
                "video_meta": {k: str(v) for k, v in video_pre_cond["meta"].items()},
                "num_video_cache_layers": len(video_kv_cache),
                "video_cache_k_shape_layer0": tuple(video_kv_cache[0]["k"].shape),
                "video_cache_v_shape_layer0": tuple(video_kv_cache[0]["v"].shape),
            }

            with open(out_dir / "idm_layout_debug.json", "w") as f:
