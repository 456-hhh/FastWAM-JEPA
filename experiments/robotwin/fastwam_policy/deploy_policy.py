...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        self._prev_action_chunk: Optional[np.ndarray] = None  # [T, D] from previous inference
        self._prev_chunk_start_step: int = 0  # episode step when prev chunk was generated
        self._episode_step: int = 0  # tracks current step within episode for ensemble
        self._ensemble_blend_weight: float = 0.3  # weight for older chunk (30% old, 70% new)
        self._ema_alpha: float = 0.85  # EMA smoothing within action queue
    ...
    def _fill_action_queue(self, observation: Dict[str, Any], instruction: str) -> None:
        ...
        chunk_start = self._episode_step

        raw_actions = []
        for i in range(n_exec):
            target_step = chunk_start + i
            action = action_chunk[i].copy()

            # Temporal ensemble: blend with previous chunk if it covers this step
            if self._prev_action_chunk is not None:
                prev_idx = target_step - self._prev_chunk_start_step
                if 0 <= prev_idx < len(self._prev_action_chunk):
                    action = (
                        (1.0 - self._ensemble_blend_weight) * action
                        + self._ensemble_blend_weight * self._prev_action_chunk[prev_idx]
                    )
            raw_actions.append(action)

        # EMA smoothing across the action queue to reduce inter-step jitter
        if len(raw_actions) > 0:
            smoothed = [raw_actions[0]]
            for i in range(1, len(raw_actions)):
                smoothed_action = self._ema_alpha * raw_actions[i] + (1.0 - self._ema_alpha) * smoothed[i - 1]
                smoothed.append(smoothed_action)
            for action in smoothed:
                self.pending_actions.append(np.asarray(action, dtype=np.float32))

        # Save current chunk for next temporal ensemble
        self._prev_action_chunk = action_chunk
        self._prev_chunk_start_step = chunk_start
    ...
    def reset(self) -> None:
        ...
        # Reset temporal ensemble state for new episode
        self._prev_action_chunk = None
        self._prev_chunk_start_step = 0
