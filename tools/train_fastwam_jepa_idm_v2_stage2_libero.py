...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Stage 2 LIBERO-only joint training for FastWAM-JEPA-IDM v2. "
            "Trains ActionDiT, JepaFuturePredictor, JEPA adapter, and proprio projection; "
            "keeps V-JEPA2 frozen."
        )
    )
...
def stage2_forward_loss(model: torch.nn.Module, sample: dict[str, Any], *, args: argparse.Namespace) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    loss_total = loss_action + float(args.lambda_future) * loss_future_jepa
...
def main() -> None:
    ...
    groups = configure_trainability(model)
