...
INFER_LATENCIES_MS = []
INFER_LATENCY_RECORDS = []
ATTN_CAPTURE_REPLAN_COUNT = 0

def _find_tokenizer_from_processor(processor):
    ...
    for obj in candidates:
        if obj is None:
            continue
        if hasattr(obj, "convert_ids_to_tokens") and callable(obj):
            return obj

    return None


def _dump_text_tokens(processor, task_description, cfg):
    if os.environ.get("FASTWAM_DUMP_TEXT_TOKENS", "0") != "1":
        return

    tok = _find_tokenizer_from_processor(processor)
    if tok is None:
        print("[TEXT TOKEN DEBUG] tokenizer not found from processor")
        return

    out_root = Path(os.environ.get("FASTWAM_ATTN_DIR", "attention_outputs"))
    out_dir = out_root / "text_token_debug"
    out_dir.mkdir(parents=True, exist_ok=True)

    suite = str(cfg.EVALUATION.task_suite_name)
    task_id = int(cfg.EVALUATION.task_id)

    max_len = int(os.environ.get("FASTWAM_TEXT_TOKEN_MAX_LEN", "129"))
    ...
    for i, tid in enumerate(input_ids):
        ...
        rows.append({
            "index": i,
            "input_id": int(tid),
            "attention_mask": int(attention_mask[i]) if i < len(attention_mask) else -1,
            "token": str(tokens[i]),
            "decoded": str(decoded),
        })
    ...
    json_path = out_dir / f"{suite}_task{task_id}_text_tokens.json"
    csv_path = out_dir / f"{suite}_task{task_id}_text_tokens.csv"
    ...
    with open(csv_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["index", "input_id", "attention_mask", "token", "decoded"])
        writer.writeheader()
        writer.writerows(rows)

    print("[TEXT TOKEN DEBUG] saved:", csv_path)
    print("[TEXT TOKEN DEBUG] task_description:", task_description)
def _sync_cuda(device):
    if torch.cuda.is_available() and str(device).startswith("cuda"):
        torch.cuda.synchronize(device)


def _latency_summary_ms(xs):
...
def _record_infer_latency_ms(infer_ms, model):
    record = {
        "index": len(INFER_LATENCIES_MS),
    ...
    INFER_LATENCIES_MS.append(float(infer_ms))
    INFER_LATENCY_RECORDS.append(record)


def _save_infer_latency_file(model, results=None):
    if len(INFER_LATENCY_RECORDS) == 0:
        return

    log_dir = Path(os.environ.get("FASTWAM_LATENCY_DIR", "latency_logs/libero"))
    ...
    records_path = log_dir / f"{model_name}_pid{pid}_{timestamp}_infer_records.jsonl"
    summary_path = log_dir / f"{model_name}_pid{pid}_{timestamp}_infer_summary.json"

    # 每次推理一行
    with open(records_path, "w", encoding="utf-8") as f:
        for record in INFER_LATENCY_RECORDS:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")

    # 汇总统计
    summary = _latency_summary_ms(INFER_LATENCIES_MS)
    summary["model"] = model_name
    summary["pid"] = pid
    summary["num_records"] = len(INFER_LATENCY_RECORDS)
    summary["records_path"] = str(records_path)

    if results is not None:
        results["infer_action_latency_ms"] = summary

    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print(f"[Latency] Saved per-inference records to: {records_path}")
    print(f"[Latency] Saved summary to: {summary_path}")
...
def _save_replan_rgb(obs, cfg, replan_idx: int):
    if os.environ.get("FASTWAM_SAVE_REPLAN_RGB", "0") != "1":
        return

    out_root = Path(os.environ.get("FASTWAM_ATTN_DIR", "attention_outputs"))
    out_dir = out_root / "replan_rgb"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    # 和模型输入一致：左边 image，右边 wrist_image
    ...
    task_id = int(cfg.EVALUATION.task_id)
    Image.fromarray(rgb).save(out_dir / f"task{task_id}_replan{replan_idx:04d}_rgb.png")
    ...
    *,
    ...
    with torch.inference_mode():
        if visualize_future_video:
            ...
            global ATTN_CAPTURE_REPLAN_COUNT

            _sync_cuda(model_device)
            t0 = time.perf_counter()

            attn_enable = os.environ.get("FASTWAM_ATTN_ENABLE", "0") == "1"
            attn_max_replans = int(os.environ.get("FASTWAM_ATTN_MAX_REPLANS", "1"))
            do_capture_attn = attn_enable and (ATTN_CAPTURE_REPLAN_COUNT < attn_max_replans)

            if do_capture_attn:
                from experiments.attention.sdpa_capture import capture_sdpa_attention

                attn_out_dir = os.environ.get("FASTWAM_ATTN_DIR", "attention_outputs/fastwam_idm")
                tag = (
                    f"task{cfg.EVALUATION.task_id}_"
                    f"replan{ATTN_CAPTURE_REPLAN_COUNT:04d}_"
                    f"{model.__class__.__name__}"
                )

                with capture_sdpa_attention(
                    model=model,
                ...
                ATTN_CAPTURE_REPLAN_COUNT += 1
            else:
                pred = model.infer_action(**infer_kwargs)

            _sync_cuda(model_device)
            infer_ms = (time.perf_counter() - t0) * 1000.0
            _record_infer_latency_ms(infer_ms, model)
    ...
    *,
    ...
    while t < max_steps + num_steps_wait:
        ...
        if len(pending_actions) == 0:
            _dump_text_tokens(processor, task_description, cfg)
            ...
            if os.environ.get("FASTWAM_SAVE_REPLAN_RGB", "0") == "1":
                _save_replan_rgb(obs, cfg, ATTN_CAPTURE_REPLAN_COUNT)
            action_chunk, imgs, predicted_future_frames = _predict_action_chunk(
                obs=obs,
                task_description=task_description,
                model=model,
...
def eval_single_process(cfg: DictConfig):
    ...
    if len(INFER_LATENCIES_MS) > 0:
        _save_infer_latency_file(model, results)
