...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FastWAM-JEPA-IDM V5 Stage2 IDM interface warm-up.")
...
def _visual_from_stage1(
    ...
    if not isinstance(config, dict):
        raise ValueError("Stage1 metadata is missing resolved_config.")
    required = (
        "visual_layers",
        "visual_hidden_dim",
        "visual_ffn_dim",
    ...
    if missing:
        raise ValueError(f"Stage1 resolved_config is missing {missing}.")
    visual = JEPAVisualDiTV5(
        num_layers=int(config["visual_layers"]),
        hidden_dim=int(config["visual_hidden_dim"]),
        ffn_dim=int(config["visual_ffn_dim"]),
        num_heads=int(config["visual_heads"]),
        attn_head_dim=int(config["visual_head_dim"]),
        use_gradient_checkpointing=True,
    )
    if not isinstance(payload.get("visual_dit"), dict):
        raise ValueError("Stage1 checkpoint is missing visual_dit weights.")
    visual.load_state_dict(payload["visual_dit"], strict=True)
    return visual.to(device=device, dtype=dtype), metadata
...
def main() -> None:
    ...
    try:
        ...
        if float(args.video_cond_noise_prob) != 0.5:
            raise ValueError("V5 Stage2 first version fixes --video-cond-noise-prob at 0.5.")
        ...
        stage1_sha = distributed_sha256(stage1_path, rank=rank)
        ...
        visual, stage1_metadata = _visual_from_stage1(
            stage1_path, device=device, dtype=parameter_dtype
        )
        verify_provenance(
            stage1_metadata,
        ...
        if action is None:
            raise RuntimeError("Stage2 release ActionDiT was not instantiated.")
        ...
        if any(parameter.requires_grad for parameter in model.action_expert.parameters()):
            raise RuntimeError("Stage2 ActionDiT must be fully frozen.")
        ...
        optimizer = torch.optim.AdamW(
            trainable, lr=args.lr_visual, weight_decay=args.weight_decay
        )
        ...
        metadata = checkpoint_metadata(
            ...
            parameter_counts={
                ...
                "action_expert": sum(p.numel() for p in model.action_expert.parameters()),
                "proprio_encoder": sum(p.numel() for p in model.proprio_encoder.parameters()),
            },
            parent_checkpoint=stage1_path,
            parent_sha256=stage1_sha,
        )
        metadata["visual_config"] = {
            "num_layers": model.visual_dit.num_layers,
            "hidden_dim": model.visual_dit.hidden_dim,
            "ffn_dim": model.visual_dit.ffn_dim,
            "num_heads": model.visual_dit.num_heads,
            "attn_head_dim": model.visual_dit.attn_head_dim,
            "vjepa_dim": model.visual_dit.vjepa_dim,
            "text_dim": model.visual_dit.text_dim,
            "spatial_pool_size": model.visual_dit.spatial_pool_size,
        }
        ...
        if args.resume_checkpoint:
            ...
            if resume_metadata.get("parent_checkpoint_sha256") != stage1_sha:
                raise ValueError("Stage2 resume parent Stage1 SHA256 mismatch.")
        ...
        rank0_print(
            ...
            f"action_parameter_count={sum(p.numel() for p in model.action_expert.parameters())}",
        )
        ...
        while global_step < args.steps:
            ...
            if global_step % args.log_every == 0:
                ...
                rank0_print(
                    ...
                    f"visual_grad_norm={float(grad_norm):.4f} lr={lr_scheduler.get_last_lr()[0]:.3e} "
