...
class Stage3VLPActionModel(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        loss, retrieval_acc = contrastive_loss(z_task, z_a, tau=tau)
        norms = latent_norms(z_v=z_v, z_l=z_l, z_p=z_p, z_task=z_task, z_a=z_a)
        return {
            "loss": loss,
            "retrieval_acc": retrieval_acc,
            "z_v_norm": loss.new_tensor(norms["z_v"]),
            "z_l_norm": loss.new_tensor(norms["z_l"]),
            "z_p_norm": loss.new_tensor(norms["z_p"]),
            "z_task_norm": loss.new_tensor(norms["z_task"]),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="runs/fastwam_jepa_idm_v3_stage3_vlp_action")
...
def _current_proprio(proprio: torch.Tensor, *, proprio_dim: int) -> torch.Tensor:
    if proprio.ndim == 3:
        current = proprio[:, 0, :]
    elif proprio.ndim == 2:
        current = proprio
    ...
    if int(current.shape[-1]) != int(proprio_dim):
        raise ValueError(f"current proprio must be [B, {int(proprio_dim)}], got {tuple(current.shape)}.")
    return current


def canonicalize_stage3_vlp_batch(
    batch: dict[str, Any],
    *,
    ...
    if int(video.shape[2]) < current_t:
        raise ValueError(f"LIBERO video has T={video.shape[2]}, but Stage3 requires at least {current_t} frames.")
    ...
    if not torch.is_tensor(proprio):
        raise ValueError("Stage3 requires LIBERO batch proprio tensor.")
    proprio = _current_proprio(proprio, proprio_dim=int(args.proprio_dim))
...
def build_frozen_vjepa_encoder(
    ...
    *,
    ...
    if any(param.requires_grad for param in encoder.parameters()):
        raise RuntimeError("V-JEPA2 encoder must be frozen for Stage3 v3.")
...
def load_stage2_checkpoint(model: Stage3VLPActionModel, checkpoint_path: str | Path, *, rank: int) -> dict[str, Any]:
    path = stage2_libero.require_file(checkpoint_path, name="--stage2-checkpoint")
    payload = torch.load(path, map_location="cpu")
    if not isinstance(payload, dict):
        raise ValueError(f"Stage2 checkpoint payload must be dict, got {type(payload)}.")
    stats: dict[str, Any] = {}
    for key, module in (
        ...
        if not isinstance(state, dict):
            raise ValueError(f"Stage2 checkpoint missing {key} state_dict.")
        module.load_state_dict(state, strict=True)
        stats[key] = {"loaded": True, "keys": len(state)}
    stage2_libero.rank0_print(rank, f"loaded_stage2_checkpoint={path}", flush=True)
    return stats
...
def save_checkpoint(
    *,
    ...
    if not isinstance(module, Stage3VLPActionModel):
...
def main() -> None:
    ...
    if int(args.current_frame_count) != 4:
        raise ValueError("Stage3 v3 expects --current-frame-count 4.")
    if int(args.vjepa_img_size) != 256:
        raise ValueError("Stage3 v3 expects --vjepa-img-size 256.")
    if int(args.raw_vjepa_tokens) != 512 or int(args.vjepa_dim) != 1408:
        raise ValueError("Stage3 v3 expects raw V-JEPA tokens [B, 512, 1408].")
    ...
    while update_step < int(args.steps):
        ...
        try:
            ...
            data_iter = iter(loader)
            raw_batch = next(data_iter)
        batch = canonicalize_stage3_vlp_batch(raw_batch, args=args, device=device, dtype=param_dtype)
