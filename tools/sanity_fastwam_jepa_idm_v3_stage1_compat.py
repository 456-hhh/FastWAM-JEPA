...
def main() -> None:
    ...
    world_tokens = torch.randn(batch_size, 16, 1408)
    ...
    assert out["loss_vlp_to_a"].ndim == 0
    assert out["loss_va_to_l"].ndim == 0
    assert torch.isfinite(out["loss_vlp_to_a"]).item()

    infer = wrapper.forward_infer(
        world_tokens=world_tokens,
        text_tokens=text_tokens,
        text_mask=text_mask,
        proprio=proprio,
    )
    assert tuple(infer["z_task_token"].shape) == (batch_size, 4, 1024)
    assert tuple(infer["z_task"].shape) == (batch_size, 1024)
    ...
    context = torch.randn(batch_size, 10, 4096)
    context_mask = torch.ones(batch_size, 10, dtype=torch.bool)
    ...
    print(
        "sanity_fastwam_jepa_idm_v3_stage1_compat passed "
